...
extern void onenet_upload_cycle(void);

struct onenet_device
{
    struct rt_onenet_info *onenet_info;

    void(*cmd_rsp_cb)(char *topic_name, uint8_t *recv_data, size_t recv_size);

} onenet_mqtt;

static void mqtt_callback(MQTTClient *c, MessageData *msg_data)
{
    char topicname[64] = { "$crsp/" };
    ...
    if (onenet_mqtt.cmd_rsp_cb != RT_NULL)
    {
        ...
        onenet_mqtt.cmd_rsp_cb(topicname, (uint8_t *) msg_data->message->payload, msg_data->message->payloadlen);
 ...
 * upload string data to OneNET cloud.
 *
 * @param   str         string data, json format
 *
 * @return  0 : upload digit data success
 *         -5 : no memory
 */
 ...
 *         -1 : function is null
 */
void onenet_set_cmd_rsp_cb(void (*cmd_rsp_cb)(char *topic_name, uint8_t *recv_data, size_t recv_size))
{
